...
interface MetricMortarMixRecord {
  userVolume: number;
...
export class MetricMortarAndMixComponent {
  ...
  userRecords: { userVolume: number, fineAggregatesKg: number, fineAggregatesSG: number, fineAggregateMC: number, cementKg: number, 
    cementSG: number, blastFurnanceSlagKg: number, blastFurnanceSlagSG: number, flyAshKg: number, flyAshSG: number, waterKg: number,
  ...
  saveToFirestore(): void {
    ...
    if (user && user.emailVerified) {
      ...
      const testCollection = collection(this.firestore, 'metricMortarMix');
  ...
  fetchRecords(): void {
    ...
    if (this.showRecords) {
      ...
      if (user && user.emailVerified) {
        ...
        const records: MetricMortarMixRecord[] = [];
        ...
        getDocs(q).then((querySnapshot) => {
          querySnapshot.forEach((doc) => {
            const data = doc.data() as MetricMortarMixRecord; // Ensure that data is typed as MortarMixRecord
  ...
  deleteRecord(record: MetricMortarMixRecord): void {
    const user = this.auth.currentUser;
    ...
    if (user && user.emailVerified) {
      ...
      getDocs(q).then((querySnapshot) => {
        if (!querySnapshot.empty) {
          ...
          const docRef = doc(this.firestore, 'metricMortarMix', docId); // Reference to the document
  ...
  populateFields(record: MetricMortarMixRecord): void {
